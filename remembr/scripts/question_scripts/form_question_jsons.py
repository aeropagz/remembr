...
# data-specific args
parser.add_argument(
    "--caption_file", type=str, default="captions_Llama-3-VILA1.5-8b_3_secs"
)
...
for i, seq_id in enumerate(seq_ids):
    ...
    try:
        ...
        print(
            f"ERROR. Questions for {seq_id} exists, however, captions do not exist. Will skip SeqID {seq_id}"
        )
        continue
    ...
    for qa_pair in unfilled_qa:
        ...
        caption_start_ids.sort(
            key=lambda x: float(x.split("/")[-1][:-4])
        )  # should already be sorted
        caption_times = np.array(
            [float(file.split("/")[-1][:-4]) for file in caption_start_ids]
        )
        ...
        for _, answer in answers.iterrows():
            ...
            current_position = np.round(
                np.array(captions[caption_idx]["position"]), 2
            ).tolist()
    ...
    out_json = {"version": 0.1, "data": all_questions}

    print(
        f"Saving data for sequence {seq_id} in ./data/questions/{seq_id}/human_qa.json"
    )
